...
namespace postProject.Bll
{
    class BranchDB : GeneralDB
    {
        ...
        public Branch SearchNameBreanch(string name)// פעולה המחזירה שם של סניף
        {
            return GetList().Find(x => x.NameB == name);
        }
        public Branch SearchNameAcityBreanch(string name,int c)// פעולה המחזירה שם של סניף
        {
            return GetList().Find(x => x.NameB == name && x.CityB == c);
        }
    }
}
